...
plot and text/PKL summaries of the results.

Outputs per vase (saved in the vase folder):
- _fits_polynomial.pdf: plot with points, fitted curves, uncertainty bands,
  and integration boundaries.
- _fit_results.pkl: pickle with coefficients, uncertainties, intersections,
  etc.
- _fit_results.txt: readable text summary of fit details and statistics.

Units: centimeters for both axes after CSV load. The CSV is expected to
contain two columns x;y with centimeter values.
"""
...
def load_points(filepath: str) -> np.ndarray:
    ...
    Returns:
    np.ndarray: N×2 array of points [x, y] in cm. Y values are converted
    to absolute values so the profile is represented on the positive side.

    Notes:
        - The importer treats the first row as a header and uses ';' as the
          delimiter. Lines starting with '#' are ignored.
    """
...
def get_min_max_x(points: np.ndarray):
    ...
    with maximum x (each a length-2 numpy array).
    """
...
def get_ranges(points: np.ndarray, split_points_indexes) -> list:
    ...
    provided split indexes. Each range is used to fit a separate polynomial.

    Args:
        points (np.ndarray): Sorted array of points (increasing x).
        split_points_indexes (List[int]): Indexes (0-based) in the sorted
            array indicating where a new segment should begin. The split point
            itself is included in the previous segment by design (+1 on end
            index).
...
def find_exact_intersections(polys, x_ranges):
    ...
    For each adjacent pair p_i, p_{i+1}, the function solves
    p_i(x) - p_{i+1}(x) = 0 and selects a root within an expanded overlapping
    x-range. If multiple valid roots are found, it picks the one closest to the
    shared boundary (midpoint between segments) to represent the stitch point.
...
def estimate_polyfit_uncertainties(x, y, degree, sigma=1.0):
    """
    Estimate coefficient uncertainties via numpy.polyfit covariance.
    ...
    Returns:
    coeffs (np.ndarray): Polynomial coefficients (length degree+1).
    coeffs_std (np.ndarray): 1-sigma uncertainties for each coefficient.
    """
...
def plot_polynomial_fits(
    ...
    """
    Fit per-range polynomials, plot data/fits/uncertainty bands, and mark
    intersections.
    ...
    Args:
        ...
        vase_name (str): Vase folder name; used for exporting the PDF.
    ...
    Notes:
        - Uncertainty band is visualized via fill_between between fits
          computed from +0.1 cm and -0.1 cm Y offsets.
        - The final PDF is saved to f"{vase_name}/_fits_polynomial.pdf".
    """
    ...
    for i, (r, degree) in enumerate(zip(ranges, degrees)):
        if len(r) > 0:
            ...
            coeffs, coeffs_std = estimate_polyfit_uncertainties(
                x_range, y_range, degree, sigma=1.0
            )
            coeffs_up, _ = estimate_polyfit_uncertainties(
                x_range, y_range + 0.1, degree, sigma=1.0
            )
            coeffs_down, _ = estimate_polyfit_uncertainties(
                x_range, y_range - 0.1, degree, sigma=1.0
            )
...
def save_results_to_pickle(results: dict, filename: str):
    """
    Save results dictionary (coefficients, uncertainties, intersections)
    to PKL.
    ...
    Notes:
                - Pickle files are Python-version dependent. For cross-platform
                    compatibility, prefer the text export as well.
    """
...
def save_fit_results_to_text(results: dict, vase_name: str):
    """
    Save a human-readable text report of fit details and uncertainties.
...
def calculate_r_squared(x, y, poly):
    ...
    Returns:
        float: R² in [0, 1], where 1 is a perfect fit.
    """
    y_pred = poly(x)
...
def main():
    """
    Load CSVs, fit polynomials per range, export plots and summaries per vase.
    """
